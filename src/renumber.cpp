...
void renumber_area( char_data *, area_data *, renumber_areas *, bool );

renumber_areas *gather_renumber_data( area_data *, int, bool );
renumber_data *gather_one_list( short, int, int, int, bool, int * );
void free_renumber_data( renumber_data * );

bool check_vnums( char_data *, area_data *, renumber_areas * );

int find_translation( int, renumber_data * );
void translate_reset( reset_data *, renumber_areas * );
void translate_objvals( char_data *, area_data *, renumber_areas *, bool );
void translate_exits( char_data *, area_data *, renumber_areas * );
void warn_progs( char_data *, int, int, area_data *, renumber_areas * );
void warn_in_prog( char_data *, int, int, char *, int, mud_prog_data *, renumber_areas * );
...
void renumber_area( char_data * ch, area_data * area, renumber_areas * r_area, bool verbose )
{
   ...
   room_index *room, *room_prev, *room_list, *room_next;
   mob_index *mob, *mob_prev, *mob_list, *mob_next;
   obj_index *obj, *obj_prev, *obj_list, *obj_next;
   int iHash;
   ...
   for( r_data = r_area->r_room; r_data; r_data = r_data->next )
   {
      ...
      else
      {
         ...
         if( room_prev == NULL )
         {
            bug( "%s: Couldn't find a room in the hash table! Skipping it.", __FUNCTION__ );
            continue;
         }
   ...
   for( r_data = r_area->r_mob; r_data; r_data = r_data->next )
   {
      ...
      else
      {
         ...
         if( mob_prev == NULL )
         {
            bug( "%s: Couldn't find a mob in the hash table! Skipping it.", __FUNCTION__ );
            continue;
         }
   ...
   for( r_data = r_area->r_obj; r_data; r_data = r_data->next )
   {
      ...
      else
      {
         ...
         if( obj_prev == NULL )
         {
            bug( "%s: Couldn't find an obj in the hash table! Skipping it.", __FUNCTION__ );
            continue;
         }
...
void warn_in_prog( char_data * ch, int low, int high, char *where, int vnum, mud_prog_data *mprog, renumber_areas * r_area )
{
...
void translate_reset( reset_data * reset, renumber_areas * r_data )
{
    ...
    */
   char *action_table[] = { "Mm1r3", "Oo1r3", "Ho1", "Po2o4", "Go1", "Eo1", "Dr1", "Rr1", "Wo8", "Zr7", NULL };
   char *p;
